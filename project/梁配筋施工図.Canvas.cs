            ...
            tb.MouseLeftButtonDown += (s, e) =>
            {
                ...
                editDia.Click += (_, __) =>
                {
                    ShowComboEditor(canvas, tb, T, wx, wy, ha, va,
                        optionsProvider, currentValueProvider, commitDia);
                };

                editHook.Click += (_, __) =>
                {
                    ShowHookLengthEditor(canvas, tb, T, wx, wy, ha, va,
                        currentHookProvider, commitHook);
                };
     ...
     LineRole role, GridBotsecozu owner, int? index = null)
        ...
        private void Redraw(Canvas canvas, GridBotsecozu item)
        {
            ...
            )
            {
                ...
                double ankaAdd = (hitLeft ? Math.Max(0, ankaLeftLen) : 0.0)
            ...
            double lastYOfUwagane = 0.0;
            {
                ...
                for (int kRow = 0; kRow < nRowsGlobal; kRow++)
                {
                    ...
                    for (int i = 0; i < spanCount; i++)
                    {
                        ...
                        if (i == 0)
                        {
                            ...
                            if (!teiUwaNow)
                            {
                                ...
                                {
                                    // Vẽ ANKA lên trên
                                    DrawLine_Rec(canvas, T, item, x1, y, x1, y + ankaUwa, Brushes.Orange, 1.2, null, "MARK");
                                    // ANKA TEXT
                                    if (anka1 == true)
                                    {
                    ...
                    for (int i = 0; i < spanCount; i++)
                    {
                        ...
                        if (i == spanCount - 1)
                        {
                            ...
                            if (!teiUwaNow)
                            {
                                if (ankaUwa > 0)
                                {
                                    DrawLine_Rec(canvas, T, item, x2, y, x2, y + ankaUwa, Brushes.Orange, 1.2, null, "MARK");
                                    // <<< ANKA TEXT >>>
                                    if (anka1 == true)
                                    {
                    ...
                    //// === NEW: Vẽ chấm đen tại ranh giới cắt thực tế sau làm tròn
                    //DrawCutDotsForRoundedChains(merged, y);
            ...
            double lastYOfShitaChu2 = 0.0;
            {
                ...
                for (int kRow = 0; kRow < nRowsGlobalShitaChu2; kRow++)
                {
                    ...
                    // --- 中央 (下宙2: cắt giữa E2, chỉ add nửa trái)
                    for (int i = 0; i < spanCount; i++)
                    {
                        ...
                        if (hasTriple)
                        {
                            double phiE2 = Math.Max(0, diaE2ShitaChu2Arr[i]);
                            double qR_useE2 = qRArr[i];
                            if (i == spanCount - 1)
                                qR_useE2 -= Math.Max(0, gTanbu) * phiE2;
                            else
                                qR_useE2 -= Math.Max(0, nTanbu) * phiE2;

                            qR_useE2 = Math.Max(qR_useE2, midArr[i] + minGap);
                            double sxR = spanRightArr[i];
                            double cE2 = 0.5 * (qR_useE2 + sxR); // cắt giữa E2
            ...
            // 下宙1 — CẮT Ở GIỮA ĐOẠN E2
            {
                ...
                for (int kRow = 0; kRow < nRowsGlobalShitaChu1; kRow++)
                {
                    ...
                    // --- 中央 (下宙1: cắt giữa E2, chỉ add nửa trái)
                    for (int i = 0; i < spanCount; i++)
                    {
                        ...
                        if (hasTriple)
                        {
                            double phiE2 = Math.Max(0, diaE2ShitaChu1Arr[i]);
                            double qR_useE2 = qRArr[i];
                            if (i == spanCount - 1)
                                qR_useE2 -= Math.Max(0, gTanbu) * phiE2;
                            else
                ...
                // 下筋 — CẮT Ở GIỮA ĐOẠN E2
                {
                    ...
                    for (int kRow = 0; kRow < nRowsGlobalShita; kRow++)
                    {
                        ...
                        for (int i = 0; i < spanCount; i++)
                        {
                            ...
                            if (hasTriple)
                            {
                                double phiE2 = Math.Max(0, diaE2ShitaArr[i]);
                                double qR_useE2 = qRArr[i];
                                if (i == spanCount - 1)
                                    qR_useE2 -= Math.Max(0, gTanbu) * phiE2;
                                else
